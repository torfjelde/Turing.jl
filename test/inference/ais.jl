...
using Turing
using Turing: AIS
using Turing.Inference: gen_logjoint, gen_logprior, gen_log_unnorm_tempered, prior_step, intermediate_step
...
@turing_testset "ais.jl" begin
    ...
    proposal_kernels_1 = [RandomWalkProposal(MvNormal(2, 1.)) for i in 1:length(schedule_1)]
    ...
    @testset "general performance" begin
        ...
        @test abs(mean(get(chn_2, :z).z) - 2.5) < 0.5 # works with 40 tempering steps, not with 10 tempering steps
        end
    end 
end
